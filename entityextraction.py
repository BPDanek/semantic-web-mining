...
def collect_metadata_tags(file_html):
    '''
    On many web documents, the <meta> tags can contain useful information about the topic of the article. However, not
    every website has them. If it doesn't we return a blank list
    '''
    ...
    try:
        ...
        # Some files do not have keyword tags. In this case, we want to extract the text via the <p> tags and perform
        # spaCy entity extraction on them.
        tags = get_text_from_paragraph_tags(file_html)
    return tags


def get_text_from_paragraph_tags(file_html):
    '''
...
def get_domain_terms_from_url(url, nlp_model):
    '''
    ...
    if response_code != 200:
        ...
        return []
    metadata_tags = collect_metadata_tags(file_html)
    if isinstance(metadata_tags, str):
        # If this function returns a string, we are dealing with an og:description tag and want to put it through the
        # spaCy entity extraction
        description = nlp_model(metadata_tags)
        # Only track some of the entities, some, like numbers and percentages, are useless. Still deliberating on using
        # NORP in this...will see later
        ...
        # If the tags are not of string type, they are a list of tags extracted from the metadata. We can return these
        # immediately
        domain_terms = metadata_tags
    # Sometimes duplicates emerge in the set of entities. Why? Who the hell knows. Let's select the 15 most common in
    # each and return them as domain terms
    return [x for x, y in Counter(domain_terms).most_common(15)]


def extract_web_domain_from_url(top_level_domains, url):
    '''
